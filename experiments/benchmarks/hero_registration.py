...
"""Compare all-gather buffer registration in restored one-rack hero training."""
...
class RegistrationArm(StrEnum):
    CONTROL = "control"
    ALLGATHER = "allgather"


def _run_registration_restored_local(config: GrugRunConfig, *, arm: RegistrationArm) -> None:
    root = Path(__file__).resolve().parents[1]
    protocol = json.loads((root / "benchmarks/registration_protocol.json").read_text())
...
def build_registration_benchmark(
    *,
    run_id: str,
    ...
    return dataclasses.replace(
        ...
        run=functools.partial(
            ...
            local_entrypoint=functools.partial(_run_registration_restored_local, arm=arm),
        ),
    )
...
@click.option("--arm", type=click.Choice([item.value for item in RegistrationArm]), required=True)
...
def main(
    ...
    return build_registration_benchmark(
        run_id=run_id,
        checkpoint=checkpoint,
        arm=RegistrationArm(arm),
        num_steps=num_steps,
        warmup_steps=warmup_steps,
        profile_steps=profile_steps,
